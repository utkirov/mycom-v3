    ...
    <div v-else-if="slides && slides.length" class="w-full px-4 sm:px-6 md:px-8">
      ...
      <div class="relative overflow-hidden rounded-[32px] shadow-2xl shadow-brand-blue/10 transform transition-all duration-500 hover:shadow-brand-blue/20">
        ...
        >
          ...
          >
            <NuxtLink :to="getBannerLink(slide)" class="block w-full h-full relative cursor-pointer" :aria-label="slide.title || 'Banner'">

              <!-- ФОН (LCP: preload для 1-го слайда) -->
              <div class="absolute inset-0 w-full h-full" data-swiper-parallax="50%">
                <NuxtImg
                    :src="slide.image"
              ...
              <div class="absolute inset-0 flex items-center px-6 md:px-12 lg:px-20">
                <div class="max-w-xl relative z-10">
                  <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/20 backdrop-blur-md border border-white/20 text-white/90 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-3 opacity-0 animate-content" style="animation-delay: 0.1s;">
                    <Icon name="ph:star-fill" class="text-yellow-400" />
                    <span>Special Offer</span>
                  </div>

                  <h2 v-if="slide.title" class="text-2xl md:text-4xl lg:text-5xl font-black text-white font-unbounded leading-[1.1] mb-6 drop-shadow-lg opacity-0 animate-content line-clamp-2" style="animation-delay: 0.2s;" data-swiper-parallax="-300">
                    {{ slide.title }}
                  </h2>

                  <div class="opacity-0 animate-content" style="animation-delay: 0.4s;" data-swiper-parallax="-200">
          ...
          <!-- Navigation Controls (FIX: Aria-labels) -->
          <div class="autoplay-progress absolute bottom-6 right-6 z-20 flex items-center justify-center w-10 h-10 md:w-12 md:h-12">
            <svg viewBox="0 0 48 48" class="w-full h-full rotate-[-90deg]">
              ...
              <circle ref="progressCircle" cx="24" cy="24" r="20" class="stroke-brand-blue fill-none stroke-[3px]" style="stroke-dasharray: 125.6; stroke-dashoffset: 0;"></circle>
            </svg>
...
const getBannerLink = (slide) => {
  if (slide.url) return slide.url.startsWith('/') ? localePath(slide.url) : slide.url;
...
:deep(.swiper-slide-active) .animate-content { animation: fadeInUp 0.8s cubic-bezier(0.2, 0.8, 0.2, 1) forwards; }
:deep(.swiper-pagination-bullet) { width: 20px; height: 3px; border-radius: 2px; background: rgba(255, 255, 255, 0.4); opacity: 1; transition: all 0.3s ease; margin: 0 3px !important; }
:deep(.swiper-pagination-bullet-active) { background: #009FE3; width: 30px; box-shadow: 0 0 10px rgba(0, 159, 227, 0.5); }
</style>